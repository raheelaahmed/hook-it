...
class PatternViewTest(TestCase):
    ...
    def test_edit_pattern_superuser(self):
        ...
        self.client.login(username="admin", password="adminpass")
        response = self.client.get(reverse('edit_pattern', args=[self.pattern.pk]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_pattern_superuser(self):
        ...
        self.client.login(username="admin", password="adminpass")
        response = self.client.post(reverse('delete_pattern', args=[self.pattern.pk]))
...
class ReviewViewTest(TestCase):
    """Tests for the Review views: edit and delete (Superuser and review owner only)."""
    ...
    def test_edit_review_owner(self):
        ...
        self.client.login(username="testuser", password="testpass")
        response = self.client.get(reverse('edit-review', args=[self.review.pk]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_review_other_user(self):
        """Ensure another user (who did not create the review) cannot edit it."""
    ...
    def test_delete_review_owner(self):
        ...
        self.client.login(username="testuser", password="testpass")
        response = self.client.post(reverse('delete-review', args=[self.pattern.pk, self.review.pk]))
        self.assertEqual(response.status_code, 302)  # Should redirect after delete
    ...
    def test_delete_review_other_user(self):
        """Ensure another user (who did not create the review) cannot delete it."""
        self.client.login(username="otheruser", password="otherpass")
        response = self.client.post(reverse('delete-review', args=[self.pattern.pk, self.review.pk]))
        self.assertNotEqual(response.status_code, 200)  # Should be redirected
